...
export class PastorService {
  ...
  async create(createPastorDto: CreatePastorDto, user: User): Promise<Pastor> {
    // TODO : primero crear el miembro y con ese ID relacionarlo y crear el pastor
  ...
  async findTerm(
    ...
    if (isUUID(term) && type === SearchType.id) {
      ...
      const listCopastores = allCopastores.filter(
        (copastor) => copastor.their_pastor?.id === term && copastor.status,
      );
      ...
      const listPreachers = allPreachers.filter(
        (preacher) => preacher.their_pastor?.id === term && preacher.status,
      );
  ...
  async update(
    ...
    // TODO : hacer aquí tmb la subida de nivel de pastor y setear en member y pastor
    // TODO : hacer condicion de rol si entra hacer la subida si no la actualización normal, hacer previas validaciones
    // NOTE : quitemos responsabilidad al member

    const { status, member_id } = updatePastorDto;
    ...
    // NOTE : tomar este ejemplo para hacer las busquedas por termino de pastor, su genero,
    // NOTE : masrital is_active, ext creando una nueva funcion como la de findMemverswithPagination,
    // NOTE : buscar pastor y hacer filter con el term de busqueda, para regresar lista de pastor, (ver indices para bsuqueda rapida.)
  ...
  async remove(id: string, user: User): Promise<void> {
    ...
    const membersByPastor = allMembers.filter(
      (member) => member.theirPastor?.id === dataPastor.id,
    );
    ...
    const promisesMembers = membersByPastor.map(async (member) => {
      await this.memberRepository.update(member.id, {
        ...
        theirCopastor: null,
        theirPreacher: null,
        theirFamilyHouse: !member.theirPreacher
          ? null
          : member.theirFamilyHouse,
        updatedAt: new Date(),
        updatedBy: user,
      });
    });

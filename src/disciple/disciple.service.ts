...
export class DiscipleService {
  ...
  async create(
    ...
    const {
      roles,
      theirPastor: their_pastor,
      theirCopastor: their_copastor,
      theirPreacher: their_preacher,
      theirFamilyHouse: their_family_home,
    } = createDiscipleDto;
    ...
    if (roles.includes('pastor')) {
      try {
        ...
        // * Aca entro defrente ala entidad para crear al pastor,
        //* Debo hacer un DTO con los mis valores del disciple
  ...
  async findTerm(
    ...
    //TODO : COLOCAR created at para filtrar de ams antiguo a mas nuevo ASC DESC
  ...
  async update(
    id: string,
    updateMemberDto: UpdateDiscipleDto,
    user: User,
  ): Promise<Disciple> {
    const {
      roles,
    ...
    if (dataMember.roles.includes('copastor') && roles.includes('copastor')) {
      ...
      const arrayMembersPreachers = allMembers.filter(
        (member) => member.theirCopastor?.id === dataCopastor.id,
      );
    ...
    if (dataMember.roles.includes('preacher') && roles.includes('preacher')) {
      ...
      if (dataMember.theirFamilyHouse && their_family_home) {
        throw new BadRequestException(
      ...
      if (
        ...
        dataMember.theirPastor?.id !== pastor.id
      ) {
        throw new BadRequestException(
          `You cannot change their_copastor to a Preacher-Member assigned to a Family House: ${dataMember.theirFamilyHouse.code_house}, Copastor: ${dataMember.theirFamilyHouse.their_copastor.member.firstName} ${dataMember.theirFamilyHouse.their_copastor.member.lastName}, if you want to change first update their_copastor in the Preacher Module`,
        );
      }
      ...
      if (
        dataMember.theirFamilyHouse === null &&
        dataMember.theirCopastor === null &&
        dataMember.theirPastor === null &&
        !dataMember.status
      ) {
        member = await this.memberRepository.preload({
          id: dataMember.id,
  ...
  async remove(id: string, user: User): Promise<void> {
    ...
    if (
      ...
      const membersByPastor = allMembers.filter(
        (member) => member.theirPastor?.id === pastorMember.id,
      );
    ...
    if (
      ...
      const membersByPastor = allMembers.filter(
        (member) => member.theirCopastor?.id === coPastorMember.id,
      );
    ...
    if (
      ...
      const membersByPastor = allMembers.filter(
        (member) => member.theirPreacher?.id === preacherMember.id,
      );

...
import { CreateCoPastorDto, UpdateCoPastorDto } from '@/copastor/dto';
...
export class CoPastorService {
  ...
  async findTerm(
    ...
    if (isUUID(term) && type === SearchType.id) {
      ...
      const listPreachers = allPreachers.filter(
        (preacher) => preacher.their_copastor?.id === term && preacher.status,
      );
  ...
  async update(
    ...
    const { their_pastor, status, member_id } = updateCoPastorDto;
